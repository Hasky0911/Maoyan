...
<div class="mainContainer1" >
  <div class="indexNavBarContainer">
    <div class="locationContainer">
      <span>郑州</span><i class="fa fa-angle-down"></i>
    </div>
  ...
  <div class="swiper-container mainSwiper">
    <div class="swiper-wrapper">
      <div class="swiper-slide">
        ...
        <!-- <router-link to = "/test">测试效果</router-link> -->
        ...
        <ul class="movieListGroup" @scroll = "listScroll">
              ...
              <div class="movieInfoContainer">
                <router-link :to="'/filminfo/'+movie.id">
                  ...
                  <div class="movieScoreContainer">
                    <div v-if="movie.sc > 0">
                      <span class="num">{{movie.sc}}</span>
                      <span v-if="movie.proScore > 0">
                        <span class="num">{{movie.proScore}}<small>分</small></span>
                      </span>
                    </div>
                    <div v-else>
...
export default {
  ...
  created() {
    ...
    this.$http({
      method: 'GET',
      url: '/API/movie/list.json?type=hot&offset=0&limit=1000',
      // url: '/api/mmdb/movie/v3/list/hot.json?ci=20&limit=1000',
      headers: {
        'X-Requested-With': 'XMLHttpRequest'
      },
      emulateJSON: true
    }).then((response) => {
      console.dir(JSON.parse(response.data).data.movies)
      ...
      that.moviesData = JSON.parse(response.data).data.movies
    }, (response) => {
      console.log('error')
    })
  },
